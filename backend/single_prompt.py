...
from resources.util import call_llm, call_openrouter_llm, setup_file_paths, process_umple_attempt, umpleCodeProcessing, umpleCodeSearch, graphVizGeneration
...
def choose_openrouter_model():
    ...
    print("3. GPT-4o Mini (openai/gpt-4o-mini)")
    print("4. Llama 3.2 3B (meta-llama/llama-3.2-3b-instruct)")
    print("5. Gemini Pro 1.5 (google/gemini-pro-1.5)")
...
def process_umple_attempt_openrouter(i: int, prompt: str, paths: dict, model: str = "anthropic/claude-3.5-sonnet") -> str:
    ...
    """
    try:
        answer = call_openrouter_llm(prompt, max_tokens=1500, temperature=0.01, model=model)
