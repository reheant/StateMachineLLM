...
from backend.resources.n_shot_examples_single_prompt_mermaid import (
...
def _resolve_example_key(system_name: Optional[str]) -> Optional[str]:
    if system_name:
        candidate = getattr(sm_descriptions, system_name, None)
...
def _resolve_ground_truth_csv_path(base_dir: str, system_name: Optional[str]) -> str:
    ground_truth_dir = os.path.join(base_dir, "resources", "ground_truth_grading")

    preset_key = _resolve_example_key(system_name)
    if preset_key:
...
def run_automatic_grading(
    ...
    args:
        student_mermaid_code: The Mermaid code generated by the student/LLM to be graded
        system_prompt: The original system prompt describing the assignment, used to find the corresponding ground-truth CSV
        system_name: Optional name of the system/assignment, used as a fallback to find the ground-truth CSV if prompt matching fails
        model: The LLM model to use for grading
        paths: A dictionary of file paths for logging and output (e.g. llm_log_path, grading_prompt_path, grading_output_path, grading_csv_path, grading_tsv_path)
        base_dir: The base directory of the project, used to resolve relative paths for ground-truth CSV files

    """
    print("Running automatic grading")
    print("Evaluating generation against ground truth")

    csv_path = _resolve_ground_truth_csv_path(base_dir, system_name)
    ...
    example_key = _resolve_example_key(system_name)
    resolved_system_description = (
        getattr(sm_descriptions, example_key)
        if example_key and isinstance(getattr(sm_descriptions, example_key, None), str)
        else system_prompt
    )
    ...
    if not ground_truth_mermaid_code:
        _append_log(
            paths.get("llm_log_path"),
            "Ground-truth Mermaid example not found for this system; grading will use only system description and CSV.\n\n",
        )
