...
from actions.EventDrivenInitialStateSearchAction import (
    EventDrivenInitialStateSearchAction,
)
from actions.EventDrivenEventSearchAction import EventDrivenEventSearchAction
from actions.EventDrivenAssociateEventsWithStatesAction import (
    EventDrivenAssociateEventsWithStatesAction,
)
from actions.EventDrivenCreateTransitionsAction import (
    EventDrivenCreateTransitionsAction,
)
from actions.EventDrivenCreateHierarchicalStatesAction import (
    EventDrivenCreateHierarchicalStatesAction,
)
from actions.EventDrivenHierarchicalInitialStateSearchAction import (
    EventDrivenHierarchicalInitialStateSearchAction,
)
from actions.EventDrivenDisplayResultsAction import EventDrivenDisplayResultsAction
from actions.EventDrivenFilterTransitionsAction import (
    EventDrivenFilterTransitionsAction,
)
from actions.EventDrivenHistoryStateSearchAction import (
    EventDrivenHistoryStateSearchAction,
)
from actions.EventDrivenFactorOutTransitionsForHierarchalStates import (
    EventDrivenFactorOutTransitionsForHierarchalStates,
)
from actions.EventDrivenParallelRegionsSearchAction import (
    EventDrivenParallelRegionsSearchAction,
)
...
def run_event_driven_smf(system_prompt):
    ...
    base_dir = os.path.join(
        os.path.dirname(__file__), "..", "resources", "Data", "event_driven_log"
    )
    ...
    diagram_base_dir = os.path.join(
        os.path.dirname(__file__), "..", "resources", "Data", "event_driven_diagrams"
    )
    ...
    n_shot_examples_event_driven = list(
    ...
    event_driven_system_name_search_action = EventDrivenSystemNameSearchAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_state_search_action = EventDrivenStateSearchAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_initial_state_search_action = EventDrivenInitialStateSearchAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_event_search_action = EventDrivenEventSearchAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_associate_events_with_states = (
        EventDrivenAssociateEventsWithStatesAction(
            belief=belief, description=system_prompt, log_file_path=log_file_path
        )
    )
    event_driven_create_transitions_action = EventDrivenCreateTransitionsAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_filter_transitions_action = EventDrivenFilterTransitionsAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_filter_transitions_action = EventDrivenFilterTransitionsAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_parallel_regions_search_action = (
        EventDrivenParallelRegionsSearchAction(
            belief=belief, description=system_prompt, log_file_path=log_file_path
        )
    )
    event_driven_create_hierarchical_states_action = (
        EventDrivenCreateHierarchicalStatesAction(
            belief=belief, description=system_prompt, log_file_path=log_file_path
        )
    )
    event_driven_hierarchical_initial_state_search_action = (
        EventDrivenHierarchicalInitialStateSearchAction(
            belief=belief, description=system_prompt, log_file_path=log_file_path
        )
    )
    event_driven_factor_out_transitions_for_hierarchal_states = (
        EventDrivenFactorOutTransitionsForHierarchalStates(
            belief=belief, description=system_prompt, log_file_path=log_file_path
        )
    )
    event_driven_history_state_search_action = EventDrivenHistoryStateSearchAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    event_driven_display_results_action = EventDrivenDisplayResultsAction(
        belief=belief, description=system_prompt, log_file_path=log_file_path
    )
    ...
    event_driven_smf = SherpaStateMachine(
        states=states,
    ...
    hierarchical_states_table = belief.get(
        "event_driven_create_hierarchical_states_action"
    )
    ...
    hierarchical_initial_states = belief.get(
        "event_driven_hierarchical_initial_state_search"
    )

    create_event_based_gsm_diagram(
