...
class EventDrivenCreateTransitionsAction(BaseAction):
    ...
    def execute(self):
        ...
        initial_state = self.belief.get("event_driven_initial_state_search_action")
        states.remove(initial_state)
        states.insert(0, initial_state)

        # get all identified events
        event_driven_events_table = self.belief.get("event_driven_event_search_action")
        events = extract_table_entries(event_driven_events_table)

        # iterate over each combination of states and events, collecting transitions tables to be merged at the end
        transitions_tables = []
        for state in states:
            for event in events:
                transitions = self.create_transition(system_name=system_name,
                                                     state=state,
                                                     event=event,
